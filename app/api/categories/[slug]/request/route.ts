...
export async function POST(
  ...
  try {
    ...
    if (insertError) {
      console.error('workshop_requests (category) insert failed:', insertError)
      return NextResponse.json({ error: '送信に失敗しました' }, { status: 500 })
    }

    try {
      const { subject, html } = generateWorkshopRequestEmail({
        workshopTitle: `[カテゴリ] ${category.name}`,
        workshopId: category.slug,
        email,
        name,
        phone,
        participants,
      ...
      console.error('category request email failed:', mailErr)
    }

    return NextResponse.json({ ok: true, request_id: requestRow.id })
  } catch (err) {
    console.error('category request handler error:', err)
